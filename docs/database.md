...
If `user_profiles` and `projects` are non-null, migration `202608100001_initial_quality_schema.sql` is already present and must not be run again. Apply only the later numbered migrations in order (`002` through `009`). Migration `009` removes the folio generator's dependency on `ON CONFLICT`, enables styled multi-line traces, and persists inspection progress. Error `42P07` from an attempted baseline rerun is harmless because the baseline is wrapped in `begin`/`commit`; discard that failed query and continue with the pending incremental files. Do not drop existing tables.
