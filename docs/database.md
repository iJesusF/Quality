...
If `user_profiles` and `projects` are non-null, migration `202608100001_initial_quality_schema.sql` is already present and must not be run again. Apply only the later numbered migrations in order (`002` through `007`). Migration `007` installs the validated RPC that persists polyline pipe traces and reports permission/no-op failures instead of showing a false success. Error `42P07` from an attempted baseline rerun is harmless because the baseline is wrapped in `begin`/`commit`; discard that failed query and continue with the pending incremental files. Do not drop existing tables.
