...
export default async function DrawingViewerPage({ params, searchParams }: { params: Promise<{ id: string }>; searchParams: Promise<{ error?: string; success?: string }> }) {
  ...
  const [{ data: signed }, { data: segments, error: segmentsError }] = await Promise.all([
    ...
    supabase.from("segments").select("id,tag,description,status,progress_percentage,drawing_geometry").eq("project_id", project.id).or(`drawing_revision_id.is.null,drawing_revision_id.eq.${revision.id}`).is("archived_at", null).order("tag"),
  ]);
  ...
  return <div className="module-page drawing-page"><section className="page-heading"><div><div className="eyebrow"><span>{drawing.number} · REV {revision.revision}</span></div><h1>{drawing.title}</h1><p>Traza la tubería sobre el plano y consulta el avance de cada tramo por color.</p></div></section>{feedback.error && <p className="form-error">{feedback.error}</p>}{feedback.success && <p className="form-success">{feedback.success}</p>}<DrawingMarkup drawingId={drawing.id} revisionId={revision.id} url={signed.signedUrl} segments={(segments ?? []).map((segment) => ({ ...segment, progress_percentage: Number(segment.progress_percentage), drawing_geometry: segment.drawing_geometry as import("@/components/drawing-markup").DrawingGeometry | null }))} /></div>;
}
