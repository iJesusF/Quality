...
type Geometry = { type: "polyline"; points: Point[] } | { type: "rectangle"; x: number; y: number; width: number; height: number };
const geometryPoints = (geometry: Geometry) => (geometry.type === "polyline" ? geometry.points : [{ x: geometry.x, y: geometry.y }, { x: geometry.x + geometry.width, y: geometry.y }, { x: geometry.x + geometry.width, y: geometry.y + geometry.height }, { x: geometry.x, y: geometry.y + geometry.height }, { x: geometry.x, y: geometry.y }]).map(({ x, y }) => `${x * 1000},${y * 1000}`).join(" ");
