...
export default async function NewInspectionPage({ searchParams }: { searchParams: Promise<{ error?: string; segment?: string }> }) {
  const [{ supabase }, { project }, { error, segment }] = await Promise.all([requireUser(), getWorkspaceContext(), searchParams]);
