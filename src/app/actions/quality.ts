...
const drawingMarkupSchema = z.object({
  ...
  geometry: z.object({
    ...
    strokes: z.array(z.object({
      ...
      color: z.string().regex(/^#[0-9a-fA-F]{6}$/), width: z.number().min(3).max(30), opacity: z.number().min(.1).max(1),
    })).min(1).max(100),
  }),
});
export async function saveSegmentDrawingMarkupAction(formData: FormData) {
  ...
  if (error) {
    const message = error.message.includes("schema cache") || error.message.includes("Could not find the function")
      ? "El backend de trazos no está instalado. Ejecuta la migración 008 y recarga el esquema de Supabase."
      : error.message;
    redirect(`/drawings/${drawingId}?error=${encodeURIComponent(message)}`);
  }
...
const deleteDrawingMarkupSchema = z.object({ segmentId: z.uuid(), drawingRevisionId: z.uuid(), drawingId: z.uuid() });
export async function deleteSegmentDrawingMarkupAction(formData: FormData) {
  const input = deleteDrawingMarkupSchema.safeParse({ segmentId: formData.get("segmentId"), drawingRevisionId: formData.get("drawingRevisionId"), drawingId: formData.get("drawingId") });
  const drawingId = String(formData.get("drawingId"));
  if (!input.success) redirect(`/drawings/${drawingId}?error=No+fue+posible+identificar+el+trazo`);
  const [{ supabase }, { project }] = await Promise.all([requireUser(), getWorkspaceContext()]);
  const { data, error } = await supabase.rpc("delete_segment_drawing_trace", {
    pid: project.id,
    drawing_ref: input.data.drawingId,
    revision_ref: input.data.drawingRevisionId,
    segment_ref: input.data.segmentId,
  });
  if (error) {
  ...
  if (!data) redirect(`/drawings/${drawingId}?error=El+trazo+no+se+pudo+eliminar`);
  revalidatePath(`/drawings/${drawingId}`); revalidatePath(`/segments/${input.data.segmentId}`);
  redirect(`/drawings/${drawingId}?success=Trazo+eliminado+permanentemente`);
}

const inspectionUpdateSchema = z.object({ inspectionId: z.uuid(), status: z.enum(["DRAFT", "IN_PROGRESS", "COMPLETED", "PENDING_REVIEW", "APPROVED", "REJECTED", "VOID"]), result: z.enum(["", "ACCEPTED", "REJECTED"]), notes: z.string().trim().max(5000) });
export async function updateInspectionAction(formData: FormData) {
  const input = inspectionUpdateSchema.safeParse({ inspectionId: formData.get("inspectionId"), status: formData.get("status"), result: formData.get("result"), notes: formData.get("notes") });
  if (!input.success) redirect(`/inspections/${formData.get("inspectionId")}?error=Revisa+los+datos+de+la+inspección`);
  const [{ supabase }, { project }] = await Promise.all([requireUser(), getWorkspaceContext()]);
  const { data, error } = await supabase.rpc("update_quality_inspection", { pid: project.id, inspection_ref: input.data.inspectionId, inspection_status: input.data.status, inspection_result: input.data.result || null, inspection_notes: input.data.notes || null });
  if (error) redirect(`/inspections/${input.data.inspectionId}?error=${encodeURIComponent(error.message)}`);
  if (!data) redirect(`/inspections/${input.data.inspectionId}?error=No+se+pudo+actualizar+la+inspección`);
  revalidatePath("/inspections"); revalidatePath(`/inspections/${input.data.inspectionId}`); revalidatePath("/segments");
  redirect(`/inspections/${input.data.inspectionId}?success=Inspección+actualizada`);
}
