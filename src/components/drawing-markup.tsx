...
import { Check, Eraser, Highlighter, MousePointer2, RotateCcw, Save, Search, Slash } from "lucide-react";
...
export function DrawingMarkup({ drawingId, revisionId, url, segments }: { drawingId: string; revisionId: string; url: string; segments: Segment[] }) {
  ...
  const [traceShape, setTraceShape] = useState<"freehand" | "straight">("freehand");
  ...
  const move = (event: PointerEvent<SVGSVGElement>) => { if (!drawing || tool !== "trace") return; const next = point(event); setActivePoints((current) => { if (traceShape === "straight") return current.length ? [current[0], next] : [next]; const previous = current.at(-1); return previous && Math.hypot(next.x - previous.x, next.y - previous.y) < .004 ? current : [...current, next]; }); };
  ...
  const clearAllLines = () => {
    if (!window.confirm(`Primera confirmación: se limpiarán las ${strokes.length} líneas del tramo ${selected?.tag}. ¿Continuar?`)) return;
    if (!window.confirm("Segunda confirmación: esta acción vacía todo el borrador del tramo seleccionado. ¿Estás completamente seguro?")) return;
    setStrokes([]); setActivePoints([]);
  };
  ...
  return <div className="plan-editor"><header className="plan-toolbar" role="toolbar" aria-label="Herramientas de marcado"><button type="button" className={tool === "select" ? "plan-tool active" : "plan-tool"} onClick={() => setTool("select")}><MousePointer2 /> Consultar</button><button type="button" className={tool === "trace" && traceShape === "freehand" ? "plan-tool active" : "plan-tool"} onClick={() => { setTool("trace"); setTraceShape("freehand"); }} disabled={!selectedId}><Highlighter /> Mano alzada</button><button type="button" className={tool === "trace" && traceShape === "straight" ? "plan-tool active" : "plan-tool"} onClick={() => { setTool("trace"); setTraceShape("straight"); }} disabled={!selectedId}><Slash /> Línea recta</button><button type="button" className="plan-tool" onClick={() => setStrokes((current) => current.slice(0, -1))} disabled={!strokes.length}><RotateCcw /> Deshacer línea</button><button type="button" className="plan-tool danger-tool" onClick={clearAllLines} disabled={!strokes.length}><Eraser /> Limpiar todo</button><div className="plan-search"><Search /><input value={query} onChange={(event) => setQuery(event.target.value)} placeholder="Buscar tramo" /></div></header>
    ...
    <div className="plan-layout"><div className="plan-stage"><iframe src={`${url}#toolbar=0&navpanes=0&view=FitH`} title="Plano PDF" /><svg className={`plan-overlay ${tool === "trace" ? "tracing" : "consulting"}`} viewBox="0 0 1000 1000" preserveAspectRatio="none" onPointerDown={begin} onPointerMove={move} onPointerUp={finish} onPointerCancel={finish}>{segments.filter((segment) => segment.drawing_geometry && segment.id !== selectedId).map((segment) => <g key={segment.id} onPointerDown={(event) => { event.stopPropagation(); selectSegment(segment.id); }}>{renderGeometry(segment.drawing_geometry!, segment.status, "saved-pipe")}<title>{segment.tag}</title></g>)}{strokes.map((stroke, index) => <polyline key={index} points={svgPoints(stroke.points)} fill="none" stroke={stroke.color} strokeWidth={stroke.width} opacity={stroke.opacity} vectorEffect="non-scaling-stroke" className="active-pipe" />)}{activePoints.length > 1 && <polyline points={svgPoints(activePoints)} fill="none" stroke={color} strokeWidth={width} opacity={opacity} vectorEffect="non-scaling-stroke" className="active-pipe" />}</svg>{tool === "trace" && <div className="trace-instruction">{traceShape === "straight" ? <><Slash /> Presiona en el inicio, arrastra hasta el final y suelta para crear una línea recta.</> : <><Highlighter /> Suelta para terminar; vuelve a presionar para continuar el mismo tramo.</>}</div>}</div>
