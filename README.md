...
Without Supabase environment variables the application runs in a clearly labelled demo mode using typed sample data. With `NEXT_PUBLIC_SUPABASE_URL` and `NEXT_PUBLIC_SUPABASE_PUBLISHABLE_KEY`, authentication and project data use Supabase.
...
Phase 1 is implemented as a production-oriented foundation. Drawing markup, inspections, Haskell template processing, materials, NCR/Punch, signatures and full offline mutation sync remain subsequent phases and are represented in the data model without being presented as complete UI features.
