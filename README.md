...
Both incremental files are safe to retry. Never fix `42P07` by dropping `user_profiles` or other production tables, because doing so can remove user, project, and quality data. See `docs/database.md` for the verification queries and recovery procedure.
